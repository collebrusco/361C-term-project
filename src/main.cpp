...
// }
// template <class GPU_Solver>
// static void gtest(GPU_Solver* solver, string name, ofstream& fout) {
// 	timer.stop_reset();
// 	float buf[64];
// 	float nbuf[64];
// 	for (int i = 0; i < 64; i++) {
...
// 		solver->forward(); solver->inverse();
// 		buf[i] = timer.stop();
// 		nbuf[i] = solver->get_last_nocpy_us();
// 	}
// 	float mean = 0., sig = 0.;
// 	for (int i = 0; i < 64; i++)
// 		mean += buf[i];
...
// 	mean /= 64.;
// 	for (int i = 0; i < 64; i++)
// 		sig += (nbuf[i]-mean) * (nbuf[i]-mean);
// 	sig /= 64.;
// 	sig = sqrt(sig);
// 	fout << name << "nocpy" << " m " << mean << " s " << sig << "\n";
// }
...
// 		for (int i = 0; i < n*n*2; i++) buff[i] = random();
// 		gtest(gpu1d, "gpu1d", fout);
// 		for (int i = 0; i < n*n*2; i++) buff[i] = random();
// 		gtest(gpu2d, "gpu2d", fout);
// 		for (int i = 0; i < n*n*2; i++) buff[i] = random();
